...
function renderLobbiesList(data) {
    ...
    $.each(data, function (index, item) {
        ...
        <div style=" min-height: 50px; padding: 15px" class="col-6">
                    ...
                    <a style="text-decoration: none;" href="/restaurant_war_exploded/drinks/${item.code}">${item.name}</a></div>
                    <div>Giá đơn vị: ${dottedMoney(item.price)}<span></span><sup>VNĐ</sup></div>
                    <div>Đơn vị tính: <span class="fw-bold">${item.unit}</span></div><br/>
                </div>
`
    });
...
$(document).ready(function () {
    ...
    const setting = {
        ...
        pageSize: limit,
        pageNumber: 1,
        dataSource: `/restaurant_war_exploded/api/v1/drinks?kw=${keyword}`,
    ...
    $('#keyword').on('keypress', function (e) {
        if (e.which === 13) {
            keyword=$('#keyword').val();
            setting.dataSource = `/restaurant_war_exploded/api/v1/drinks?kw=${keyword}`
            container.pagination(setting)
